...
fn pt2(input: &str) -> usize {
    ...
    for brick in bricks_by_z.iter().flatten() {
        ...
        disintegrate_bricks(brick.2, &supported_bricks, &supporting_bricks, &mut disintegrated_bricks);
...
fn disintegrate_bricks(brick_label: usize, supported_bricks: &HashMap<usize, HashSet<usize>>, supporting_bricks: &HashMap<usize, HashSet<usize>>, disintegrated_bricks: &mut HashSet<usize>) {
    ...
    for supported_brick in supported_bricks_entry {
        let supporting_bricks_entry = supporting_bricks.get(&supported_brick).unwrap();
        if supporting_bricks_entry.difference(disintegrated_bricks).collect::<HashSet<_>>().len() == 0 {
    ...
    for brick in next_bricks_to_check {
        disintegrate_bricks(brick, supported_bricks, supporting_bricks, disintegrated_bricks);
    }
}
